...
describe('UI state integration', () => {
  let consoleSpy: ReturnType<typeof vi.spyOn>;
  let state: ReturnType<typeof createFormatterState>;
  ...
  describe('AC-6: Narrow terminal fallback', () => {
    it('falls back to existing formatter when terminal width < 70', () => {
      // Set narrow terminal
      process.stdout.columns = 69;
      ...
      formatMessage(
        createToolUseMessage(
          'Task',
          { subagent_type: 'agent', description: 'Test task' },
          'task-1'
        ),
        state,
        'normal',
      ...
      // Verify task registered in formatter state but NOT in UI state
      expect(state.activeTasks.size).toBe(1);
      // UI state should not be updated due to narrow terminal
      expect(uiState.agents.size).toBe(0);
    });
  });

  describe('AC-7: No color support fallback', () => {
    it('falls back to existing formatter when TERM lacks color support', () => {
      // Set terminal with no color support
      ...
      // Verify task registered in formatter state but NOT in UI state
      expect(state.activeTasks.size).toBe(1);
      // UI state should not be updated due to no color support
      expect(uiState.agents.size).toBe(0);
    });
  });

  describe('UI activation conditions', () => {
    it('activates UI when verbosity normal, width >= 70, and color support', () => {
      process.stdout.columns = 80;
      process.env['TERM'] = 'xterm-256color';
      ...
      // With proper conditions, UI should be used (registerAgent would be called)
      // We verify the task is registered in both states
      expect(state.activeTasks.size).toBe(1);
      expect(uiState.agents.size).toBe(1);
    });

    it('does not activate UI when verbosity is quiet', () => {
      process.stdout.columns = 80;
      process.env['TERM'] = 'xterm-256color';
